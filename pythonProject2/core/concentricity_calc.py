...
class ConcentricityCalculator:
    ...
    def __init__(self, pixel_to_mm_ratio: float = 0.1):
        ...
        self.current_result = None  # 添加当前结果
    ...
    def calculate(self, inner_circle, outer_circle, **kwargs):
        ...
        # 创建简洁的检测结果记录
        detection_record = {
            'detection_time': result.get('detection_time', datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            'concentricity': result.get('concentricity_permille', 0),
            'is_qualified': result.get('is_within_tolerance', False),
            'error_mm': result.get('eccentricity_mm', 0),
            'pixel_error': result.get('pixel_eccentricity', 0),
            'inner_center': {
                'x': inner_circle.get('x', 0),
                'y': inner_circle.get('y', 0),
                'area': inner_circle.get('area', 0)
            } if isinstance(inner_circle, dict) else inner_circle,
            'outer_circle': {
                'x': outer_circle.get('x', 0),
                'y': outer_circle.get('y', 0),
                'radius': outer_circle.get('radius', 0)
            } if isinstance(outer_circle, dict) else outer_circle,
            'timestamp': result.get('timestamp', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        }

        # 保存到历史记录
        self.results_history.append(detection_record)
        self.current_result = detection_record

        logger.info(f"已保存第{len(self.results_history)}条检测记录到历史")
